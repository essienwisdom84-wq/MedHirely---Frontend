...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginBtn) {
        ...
        loginBtn.addEventListener('click', async (e) => {
            ...
            console.log("📦 Dispatching login credential package...", { email: emailVal });

            // 3. Map out the login route pointing to your config.js asset variables
            const loginUrl = window.APP_CONFIG?.AUTH?.LOGIN || 'https://medhirely-backend.onrender.com/api/auth/login';
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert('🎉 Login successful! Swapping to shift posting panel...');
                    
                    // 5. Safe side-by-side redirect since new-shift.html sits right next to it!
                    window.location.href = "new-shift.html";
                } else {
                    alert(`Login Refused: ${data.message || 'The email or password typed is invalid.'}`);
                }

            } catch (err) {
                console.error("❌ Gateway handshake failure:", err);
                alert("Could not reach authentication servers. Wait a moment for Render to boot.");
            } finally {
