...
doneBtn.addEventListener("click", () => {
  doneBtn.innerText = "Saving Choice...";
  doneBtn.className =
    "w-64 bg-slate-700 text-white font-bold text-sm py-3 rounded-lg shadow-md transition pointer-events-none";

  setTimeout(() => {
    statusModal.classList.replace("scale-100", "scale-95");
    statusModal.classList.replace("opacity-100", "opacity-0");
  ...
  setTimeout(() => {
    statusModal.classList.replace("scale-95", "scale-100");
    statusModal.classList.replace("opacity-0", "opacity-100");
